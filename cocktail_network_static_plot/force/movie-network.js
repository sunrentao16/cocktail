...
/* .......................................................................... */
  ...
  var force = d3.layout.force()
    .charge(-320)
    .size( [WIDTH, HEIGHT] )
    .linkStrength( function(d,idx) { return d.weight/50; } )
    .linkDistance(d=> 1/d.weight);
	...
	function create_table(neb)
	{
		...
		*/
		...
		for (var i = 0; i < 10; ++i)
		{
		    newHTML += "<tr>";
		    for (var j = 0; j < 1; ++j)
		        newHTML += "<td> <font size = '3.5' >"+ neb[i] +"</font> </td>";
		    newHTML += "</tr>";
		}
		newHTML += "</table>";
		return newHTML;
  ...
  */
  function getMovieInfo( n, nodeArray ) {
    ...
    if( n.cover )
      ...
      info += '<div class=t style="float: right" >' + '<font color="red"; size="5">' + "Ingredient:  " + n.title  +'</font>' +'</div>';
      //var str = n.neighbor;
      //var tbl = str.split(',');
      
      info += '<div class=t style="float: right">' + "Top connected Ingredients:  " + create_table(n.neighbor)+ '</div>';

	  //info += '<div class=t style="float: right">' + "Top connected Ingredients:  " + n.neighbor[2] + '</div>';
    info +=
    '<img src="close.png" class="action" style="top: 0px;width:25px;height:25px;" title="close panel" onClick="toggleDiv(\'movieInfo\');"/>' +
    '<img src="target-32.jpg" class="action" style="top: 280px;width:30px;height:25px;" title="center graph on movie" onclick="selectMovie('+n.index+',true);"/>';
  ...
  d3.json(
    ...
    var node_size = d3.scale.linear()
      .domain([5,500])	// we know score is in this domain
      .range([1,16])
      .clamp(true);
    ...
    var graphNodes = networkGraph.append('svg:g').attr('class','grp gNodes')
      ...
      .style('fill', function(d){ return "hsl(" + Math.random() * 360 + ",85%,50%)" })
      //.on("click", function(d) { highlightGraphNode(d,true,this); } )    
      .on("click", function(d) { showMoviePanel(d); } )
      //.on("click", function(d) { return name.attr(d.id) } )
